...
try:
    if settings.supabase_url and settings.supabase_service_role_key:
        ...
        class MockUser:
            def __init__(self, email, user_id, tenant_id, name):
                self.id = user_id
                self.email = email
                self.app_metadata = {"role": "user", "tenant_id": tenant_id}
        ...
        class ChallengeAuth:
            def get_user(self, token):
                # Local challenge sessions obey the same signature, audience,
                # and expiration checks as the request authentication layer.
                try:
                    payload = jwt.decode(
                        token, settings.secret_key, algorithms=["HS256"],
                        audience="authenticated", options={"require": ["exp", "aud"]},
                    )
                    email = payload.get("email")
                    if email:
                        users = self.list_users()
                        for u in users:
                            if u.email == email and u.id == payload.get("id"):
            ...
            def list_users(self):
                # Only the assignment's two accounts exist in local mode.
                return [
                    MockUser("sunset@propertyflow.com", "user-sunset", "tenant-a", "Sunset Properties Manager"),
                    MockUser("ocean@propertyflow.com", "user-ocean", "tenant-b", "Ocean Rentals Manager"),
                ]
